...
const AutorErrores = Yup.object().shape({
    nombre: Yup.string()
        .min(5, "Debe tener como minimo 5 caracteres")
            ...
            {({ errors, touched, isValid, dirty }) => (
                <Form>
                    ...
                    {touched.nombre && errors.nombre && (
                        <div className="ms-3 mt-1 text-danger">{errors.nombre}</div>
                    )}
                    <Field name="apellido" className="form-control mt-2" placeholder="Apellido"/>
                    {touched.apellido && errors.apellido && (
                        <div className="ms-3 mt-1 text-danger">{errors.apellido}</div>
                    )}
                    <Link className="btn btn-primary m-2 " to="/autores">Volver</Link>
